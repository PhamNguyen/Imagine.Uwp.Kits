...
namespace Imagine.Uwp.Kits.Services
{
    public class StorageService
    {
        ...
        public static async Task<T> Load<T>(object fileName)
        {
            // this reads XML content from a file ("filename") and returns an object  from the XML
            T objectFromXml = default(T);
            var serializer = new XmlSerializer(typeof(T));

            IStorageItem item = await localFolder.TryGetItemAsync(fileName.ToString());
            if (item != null)
            {
        ...
        public static async Task Save<T>(object fileName, T data)
        {
            StorageFile storageFile = await localFolder.CreateFileAsync(fileName.ToString(), CreationCollisionOption.ReplaceExisting);
